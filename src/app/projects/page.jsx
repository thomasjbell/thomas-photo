          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 border border-mono-200 dark:border-mono-400 p-4 md:p-6 bg-white dark:bg-mono-500 drop-shadow-lg rounded-2xl">
            ...
            {orderedItems.map((item, index) => (
              ...
              >
                ...
                {item.slug ? (
                  <a href={`/projects/${item.slug}`} className="block relative w-full aspect-video overflow-hidden group">
                    <Image
                      src={item.imagePath}
                ...
                <div className="flex flex-col flex-1 p-5 gap-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.technologies.map((tech) => (
                      <span key={tech} className="px-2.5 py-1 text-xs font-semibold rounded-full bg-mono-100 dark:bg-mono-500 text-mono-400 dark:text-mono-200 border border-mono-200 dark:border-mono-400">
                        {tech}
                      </span>
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-2 pt-1">
                    {item.links?.map((link) => (
                      <Button key={link.href} href={link.href} variant={link.variant === "slate" ? "mono" : "mono-outline"} size="card" className="flex-1" target="_blank" rel="noopener noreferrer">
                        {link.label}
                      </Button>
                    ))}
                    {item.slug && (
                      <Button href={`/projects/${item.slug}`} variant="mono-outline" size="card" className="flex-1">
                        Read More
                      </Button>
                    )}
                  </div>
